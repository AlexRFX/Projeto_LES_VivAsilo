...
try {
    $pdo = db_connect();
    $stmt = $pdo->prepare('UPDATE tb_mantenedor SET mantenedor_telefone = :telefone WHERE mantenedor_fk = :id') OR die("Error:".mysql_error());
    $stmt->execute(array(':id' => $_SESSION['user_id'], ':telefone' => $telefone));
