    ...
    <body bgcolor="#000000">
        ...
        // O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta foto:
            ...
            if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
                ...
                $resizeObj = new resize('../imgs/imgsuser/'.$_SESSION['user_id'].'.jpg');

                // *** 2) Redimensionar imagens (Opções: exato, retrato, paisagem, auto, crop)
                $resizeObj -> resizeImage(200, 220, 'exact');

                // *** 3) Salvar imagem
                $resizeObj -> saveImage('../imgs/imgsuser/'.$_SESSION['user_id'].'.jpg', 1000);
                $pdo = db_connect();
                // Upload efetuado com sucesso, exibe a mensagem:
                $stmt = $pdo->prepare("UPDATE tb_mantenedor SET mantenedor_foto = :linkimg WHERE mantenedor_fk = :id");
